...
class HyLinUCB(Algorithm):
    def __init__(self, arms, delta, M, N, S1, S2, sigma, lmbda, info=None):
        ...
        for i in range(self.L):
            ...
            self.W_arr_inv.append((1 / self.lmbda)*np.eye(self.k))
        ...
        self.V_tilde_inv = (1 / self.lmbda) * np.eye(self.d)
        self.t = 0
        self.a_t = 0
    ...
    def ucb_bonus(self, i, a=None):
        if a is None:
            a = self.arms[i]
        s_i = np.dot(a[0], np.dot(self.V_tilde_inv, a[0])) -\
                2*np.dot(a[0], np.dot(self.V_tilde_inv @ self.B_arr[i] @ self.W_arr_inv[i], a[1])) +\
                np.dot(a[1], np.dot(self.W_arr_inv[i], a[1])) +\
                np.dot(a[1], np.dot(self.W_arr_inv[i] @ self.B_arr[i].T @ self.V_tilde_inv @ self.B_arr[i] @ self.W_arr_inv[i], a[1]))
    ...
    def update(self, reward, regret, arm_set):
        ...
        self.u += np.dot(self.B_arr[self.a_t] @ self.W_arr_inv[self.a_t], \
                   self.v_arr[self.a_t])
        self.V_tilde = self.V_tilde + \
                        self.B_arr[self.a_t] @ self.W_arr_inv[self.a_t] @ self.B_arr[self.a_t].T
        self.B_arr[self.a_t] += x_t_vec @ z_t_vec.T
        self.W_arr[self.a_t] += z_t_vec @ z_t_vec.T
        self.W_arr_inv[self.a_t] = np.linalg.inv(self.W_arr[self.a_t])
        self.v_arr[self.a_t] += reward * z_t_vec.reshape(-1)
        self.V_tilde += (x_t_vec @ x_t_vec.T - \
                         self.B_arr[self.a_t] @ self.W_arr_inv[self.a_t] @ self.B_arr[self.a_t].T)
        self.u += reward * x_t_vec.reshape(-1) - \
                    np.dot(self.B_arr[self.a_t] @ self.W_arr_inv[self.a_t], self.v_arr[self.a_t]).reshape(-1)
        self.V_tilde_inv = np.linalg.inv(self.V_tilde)
        self.theta_hat = np.dot(self.V_tilde_inv, self.u)
        for i in range(self.L):
            self.beta_hat_arr[i] = np.dot(self.W_arr_inv[i], \
                                        self.v_arr[i] - \
