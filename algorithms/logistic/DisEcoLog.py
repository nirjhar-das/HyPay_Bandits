...
class DisEcoLog(Algorithm):
    ...
    def get_ucb_index(self, idx):
        ...
        bonus = self.conf_radius(idx) * np.sqrt(np.dot(self.arms[idx], np.dot(self.W_inv_arr[idx], self.arms[idx])))
    ...
    def create_ecolog_problem_def(self):
        theta = cp.Variable((self.d + self.k, 1), name='theta')
        theta_proxy = cp.Variable((self.d + self.k, 1), name='theta_proxy')
        theta_proxy_rt = cp.Variable((self.d + self.k, 1), name='theta_proxy')
        arm = cp.Parameter((self.d + self.k, 1), name='arm')
        theta_hat = cp.Parameter((self.d + self.k, 1), name='theta_hat')
        V_sqrt = cp.Parameter((self.d + self.k, self.d + self.k), name='V_matrix', symmetric=True)
        W_sqrt = cp.Parameter((self.d + self.k, self.d + self.k), name='W_matrix', symmetric=True)
        r_t = cp.Parameter(name='reward')
        gamma = cp.Parameter(name='gamma')
        quad_form_V = cp.sum_squares(V_sqrt @ theta_proxy)
        ellipse = quad_form_V <= gamma
    ...
    def solve_ecolog_problem(self, idx, reward):
        self.ecolog_problem.param_dict['theta_hat'].value = self.theta_hat_arr[idx].reshape(-1 , 1)
        self.ecolog_problem.param_dict['arm'].value = self.arms[idx].reshape(-1, 1)
        self.ecolog_problem.param_dict['W_matrix'].value = sc.linalg.sqrtm(self.W_arr[idx])
        self.ecolog_problem.param_dict['V_matrix'].value = sc.linalg.sqrtm(self.V_arr[idx])
        self.ecolog_problem.param_dict['reward'].value = reward
        self.ecolog_problem.param_dict['gamma'].value = self.conf_radius(idx)**2
        self.ecolog_problem.solve(solver=cp.SCS, eps=(1.0 / (self.t_i_arr[idx] + 1.0)))
        ...
        if (dsigmoid(np.dot(arm, theta_0)) <= 2.0 * dsigmoid(np.dot(arm, theta_bar))) \
            ...
            if reward == 1.0:
                ...
                self.W_inv_arr[self.a_t] -= dsigmoid(np.dot(theta_0, arm)) * np.outer(np.dot(self.W_inv_arr[self.a_t], arm), \
                                                     np.dot(self.W_inv_arr[self.a_t], arm)) / (1 + dsigmoid(np.dot(theta_0, arm)) *\
                                                                                               np.dot(arm, np.dot(self.W_inv_arr[self.a_t], arm)))
