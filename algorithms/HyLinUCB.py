...
class HyLinUCB(Algorithm):
    ...
    def get_reward_estimate(self, i):
        reward = np.dot(self.arms[i][0], self.theta_hat) +\
                    np.dot(self.arms[i][1], self.beta_hat_arr[0]) +\
                    self.p_beta(i) * np.sqrt(np.dot(self.arms[i][0], np.dot(self.V_tilde, self.arms[i][0]))) +\
                    self.q_theta() * np.sqrt(np.dot(self.arms[i][0], np.dot(self.W_arr, self.arms[i][1])))
        return reward
    ...
    def update(self, reward, regret):
        ...
        self.V_tilde += self.B_arr[self.a_t] @ np.linalg.inv(self.W_arr[self.a_t]) @ self.B_arr[self.a_t].T
        self.t += 1

    def save_results(self):
        df = pd.DataFrame(data = {'reward': self.rewards, 'regret': self.regrets})
        df.to_csv(f'{self.name}_Result.csv')